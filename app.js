...
async function route(){ const {path,params}=parseHash(); const h=path||'#home'; setActive(h);
  ...
  else if(h==='#profile'){ goto('#profile'); await Admin.render(); } try{ const Settings = await import('./settings.js'); Settings.wireProfileSettings(); }catch(_){}
  else if(h==='#checkin'){ goto('#checkin'); await Checkin.render(); } if(document.getElementById('checkinSummaryCards')){ try{ await Checkin.renderSummaryCards(); }catch(_){}} }
