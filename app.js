...
async function route(r){ const {path,params}=r||parseHash(); const h=path||'#home'; Object.values(views).forEach(v=>v.classList.add('hide')); active(h);
  if(h==='#home'){ views.home.classList.remove('hide'); await News.renderHome(); await Links.renderHome(); }
  ...
  else if(h==='#links'){ views.links.classList.remove('hide'); await Links.render(); } }
