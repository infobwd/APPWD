...
async function route(){ const {path,params}=parseHash(); const h=path||'#home'; setActive(h);
  ...
  else if(h==='#profile'){ try{ const S=await import('./settings.js'); S.wireProfileSettings?.(); }catch(_){}  goto('#profile'); await Admin.render(); }
...
document.addEventListener('appwd:postSaved', async (ev)=>{
  try{
    ...
    const {path} = parseHash?.()||{path:location.hash};
