...
const getLineId = () => { try{ const prof=JSON.parse(localStorage.getItem('LINE_PROFILE')||'null'); return prof?.userId||null; }catch{ return null; } };
...
function mountAdvanced(){
  ...
  if(!section){ section=document.createElement('section'); section.id='profile-advanced'; section.className='mt-6'; section.innerHTML=`
    ...
    <div class='mt-3 flex items-center justify-between p-4 rounded-xl border border-slate-700 bg-slate-900/40'>
      ...
      <button id='swToggleBtn' class='px-4 py-2 rounded-lg border border-slate-600 hover:bg-slate-800 transition'>…</button>
...
export async function render(){
  ...
  if(byId('btnSaveSettings')) byId('btnSaveSettings').onclick=async()=>{
    const payload={ CHECKIN_START:byId('set_CHECKIN_START')?.value||'07:30', CHECKIN_ON_TIME_UNTIL:byId('set_CHECKIN_ON_TIME_UNTIL')?.value||'08:00', SUMMARY_DEFAULT_RANGE_DAYS:Number(byId('set_SUMMARY_DEFAULT_RANGE_DAYS')?.value||30), SLIDER_AUTO_MS:Number(byId('set_SLIDER_AUTO_MS')?.value||4000), BRAND_TITLE:byId('set_BRAND_TITLE')?.value||'APPWD', BRAND_LOGO_URL:byId('set_BRAND_LOGO_URL')?.value||'' };
...
window.addEventListener('hashchange', async ()=>{ const admin=await isAdmin(); if(admin) mountAdvanced(); else document.getElementById('profile-advanced')?.remove(); });
