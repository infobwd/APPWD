...
function wireButtons(){ els.btnIn().onclick = () => handleCheck('in'); els.btnOut().onclick = () => handleCheck('out'); }
async function handleCheck(type){
  ...
  const { error } = await supabase.from('checkins').insert({ user_id: user.id, type, lat, lng, photo_path });
...
async function refreshList(){
  ...
  const { data, error } = await supabase.from('checkins').select('id,type,ts,photo_path,lat,lng').eq('user_id', user.id).order('ts', { ascending:false }).limit(20);
  if(error){ list.innerHTML = '<div class="text-red-300">โหลดประวัติไม่สำเร็จ</div>'; return; }
  const rows = await Promise.all((data||[]).map(async (r)=>{
    let img=''; if(r.photo_path){ const url = await signedUrl('checkin-photos', r.photo_path, 3600); if(url){ img = `<img src="${url}" class="w-12 h-12 rounded object-cover border border-[#223052]" alt="">`; } }
    ...
    return `<div class="flex items-center gap-3 p-2 border border-[#223052] rounded-lg">${img}<div><div class="font-medium">${r.type === 'in' ? 'เช็คอิน' : 'เช็คเอาท์'} • ${time}</div>${pos}</div></div>`;
  }));
