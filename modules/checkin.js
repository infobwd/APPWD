...
function dist(a,b,c,d){ const R=6371000; const toR=x=>x*Math.PI/180; const dLat=toR(c-a), dLon=toR(d-b); const h=Math.sin(dLat/2)**2+Math.cos(toR(a))*Math.cos(toR(c))*Math.sin(Math.min(1,Math.max(-1,Math.sin((d-b)/2))))**2; return 2*R*Math.asin(Math.sqrt(Math.sin(dLat/2)**2+Math.cos(toR(a))*Math.cos(toR(c))*Math.sin((d-b)*Math.PI/360)**2)); }
async function doCheckin(method){ const profile=JSON.parse(localStorage.getItem('LINE_PROFILE')||'null'); if(!profile){ toast('ต้องเข้าสู่ระบบด้วย LINE ก่อน'); return; } if(!lastGeo){ toast('ยังไม่ได้ตำแหน่ง'); return; }
  ...
  if(within===false){ // ไม่อนุญาตเช็คอิน "มาทำงาน" ให้เลือกเหตุผล
