...
// PATCH: toast fallback to SweetAlert2/alert
function toastSafe(msg, type='success'){
  try{ if(typeof toast==='function'){ toast(msg); return; } }catch(_){}
  if (window.Swal) { Swal.fire({toast:true, position:'top', icon:(type==='error'?'error':'success'), title: String(msg), timer: 2000, showConfirmButton:false}); }
...
window.deleteCheckin = function(id){
  ...
  document.getElementById('okDel').onclick = async ()=>{
    ...
    if(res.error){ toastSafe('ลบไม่สำเร็จ','error'); return; }
    closeSheet();
    toastSafe('ลบแล้ว');
    await loadToday();
  };
