...
////////////////////////////////////////////////////
// === PATCH: #checkin — map + fixed FAB + toast + dedupe (2025-09-17) ===
(function(){
  const supa = window.supabase || window.SUPABASE || window.db;

  // ---------- CSS: FAB positions + flicker hardening
  try {
    const style = document.createElement('style');
    style.id = 'checkin-fab-style';
    style.innerHTML = `
      #checkinMap { height: 280px; border-radius: 12px; margin: .5rem 0 1rem; position: relative; z-index:1; }
      .fab { position: fixed; z-index: 1000; pointer-events: auto; touch-action: manipulation;
             will-change: transform; backface-visibility: hidden; -webkit-font-smoothing: antialiased; }
      #btnOpenScanner.fab { left: 1rem;  top: 1rem;  }
      #btnCheckin.fab     { right:1rem;  top: 1rem;  }
      #btnRefreshGeo.fab  { left: 1rem;  bottom:1rem; }
      #btnGpsOnly.fab     { right:1rem;  bottom:1rem; }
      /* กัน overlay ของ Leaflet มาบังกดปุ่ม */
      .leaflet-pane, .leaflet-map-pane { z-index: 10; }
    `;
    document.head.appendChild(style);
  } catch(err) { console.warn('FAB style inject failed', err); }

  // ติด class .fab ให้ปุ่ม (ถ้ามี)
  ['btnOpenScanner','btnCheckin','btnRefreshGeo','btnGpsOnly','btnCheckinGPS','btnReloadGPS'].forEach(id=>{
    const el = document.getElementById(id);
    if (el && !el.classList.contains('fab')) el.classList.add('fab');
  });

  // ---------- โหลดค่าพิกัดจาก settings (fallback global)
  let schoolCfg = null;
  async function getSchoolConfig(){
    if (schoolCfg) return schoolCfg;
    const keys = ['SCHOOL_LAT','SCHOOL_LNG','SCHOOL_RADIUS_METERS'];
    try{
      const { data } = await (supa?.from ? supa.from('settings').select('key,value').in('key', keys) : Promise.resolve({data:null}));
  ...
  function injectLeaflet(){
    return new Promise((resolve)=>{
      if (window.L && L.map) return resolve(true);
      const css = document.createElement('link');
      css.rel = 'stylesheet';
      css.href = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.css';
      const js  = document.createElement('script');
      js.src = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.js';
      let done = 0; const ok = ()=>{ if (++done===2) resolve(true); };
      css.onload = ok; js.onload = ok;
      css.onerror = ()=>resolve(false); js.onerror = ()=>resolve(false);
      document.head.appendChild(css); document.head.appendChild(js);
    });
  }

  // ---------- Map + markers (โรงเรียน/เรา)
  async function ensureMap(){
    const ok = await injectLeaflet();
    const cfg = await getSchoolConfig();

    // สร้าง container ถ้ายังไม่มี (หลัง #geoState)
    let box = document.getElementById('checkinMap');
    if (!box){
      box = document.createElement('div'); box.id = 'checkinMap';
      const ref = document.getElementById('geoState') || document.getElementById('checkinList') || document.body;
      ref.parentNode.insertBefore(box, (ref.id==='geoState') ? ref.nextSibling : ref);
    }
    if (!ok || !window.L) return null;

    if (!window._checkinMap){
      const m = L.map('checkinMap', { zoomControl:true, attributionControl:false }).setView([cfg.lat||0, cfg.lng||0], 16);
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom:20 }).addTo(m);

      const schoolMarker = L.marker([cfg.lat, cfg.lng], { title:'ตำแหน่งโรงเรียน' }).addTo(m);
      const schoolCircle = L.circle([cfg.lat, cfg.lng], { radius: cfg.radius||150, color:'#22c55e', fillColor:'#22c55e', fillOpacity:0.08 }).addTo(m);

      const userMarker = L.marker([cfg.lat, cfg.lng], { title:'ตำแหน่งของฉัน', opacity:0.0 }).addTo(m);

      window._checkinMap = { m, schoolMarker, schoolCircle, userMarker, fitted:false };
    }else{
      window._checkinMap.schoolCircle.setRadius(cfg.radius||150);
      window._checkinMap.schoolMarker.setLatLng([cfg.lat, cfg.lng]);
    }
    return window._checkinMap;
  }
  ...
  async function updateGeoState(){
    ...
    navigator.geolocation.getCurrentPosition(async pos=>{
      ...
      const ctx = await ensureMap();
      if (ctx && ctx.userMarker){
        ctx.userMarker.setLatLng([cur.lat, cur.lng]).setOpacity(1.0);
        if (!ctx.fitted){
          const group = L.featureGroup([ctx.schoolMarker, ctx.userMarker]);
          ctx.m.fitBounds(group.getBounds().pad(0.3));
          ctx.fitted = true;
        }
      }
  ...
  // ---------- กันเช็คอินซ้ำ (วันเดียวกัน งานปกติ)
  ...
  async function doCheckin(){
    ...
    try{
      if (await hasCheckedInToday()){ toastErr('เช็คอินวันนี้ไปแล้ว (งานปกติ)'); return; }
  ...
  // ---------- ผูกปุ่ม 4 มุม (แบบไม่ทำให้กระพริบ)
  ...
  function bindButtons(){
    bindOnce('btnOpenScanner', ()=>{ if (typeof openScanner==='function') openScanner(); });
    bindOnce('btnCheckin', ()=>doCheckin());
    bindOnce('btnRefreshGeo', ()=>updateGeoState());
    bindOnce('btnGpsOnly', ()=>doCheckin());
    // legacy ids เผื่อหน้าเก่า
    bindOnce('btnCheckinGPS', ()=>doCheckin());
    bindOnce('btnReloadGPS',  ()=>updateGeoState());
  }
