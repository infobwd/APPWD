...
export async function render(){
  // Arrange geo action buttons
  try{
    const btnGps=document.getElementById('btnGpsOnly');
    const btnRefresh=document.getElementById('btnRefreshGeo');
    if(btnGps && btnRefresh && btnGps.parentElement===btnRefresh.parentElement){
      const wrap=btnGps.parentElement;
      wrap.classList.add('grid','grid-cols-2','gap-2');
      btnRefresh.style.order='1';
      btnGps.style.order='2';
      btnGps.classList.add('justify-self-end');
    }
  }catch(_){ }
...
function deleteCheckin(id){
  ...
  supabase.from('checkins').delete().eq('id', id).then(res=>{
...
function showCheckinSuccess(){
  const root = document.getElementById('checkinView');
  if(!root) return;
  let el = document.getElementById('checkinSuccess');
  if(!el){
    el = document.createElement('div');
    el.id = 'checkinSuccess';
    el.className = 'p-3 mb-2 rounded-lg border text-sm';
    el.style.background = '#e6ffed';
    el.style.borderColor = '#a7f3d0';
    el.style.color = '#065f46';
    el.innerHTML = '✅ เช็คอินแล้วเรียบร้อย';
