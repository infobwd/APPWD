...
async function renderSummary(){
  ...
  async function collect(since){
    let base = supabase.from('checkins')
      .select('id,category,created_by,line_user_id',{head:false})
      .gte('created_at', since.toISOString());
    // try with filter; if 400 (type mismatch) fallback to base (overall)
...
export function initTabs(){
  const tabs = document.querySelectorAll('[data-checkin-tab]');
  tabs.forEach(el=>{
    el.addEventListener('click', ()=>{
      const cat = el.getAttribute('data-checkin-tab')||'all';
      tabs.forEach(t=>t.classList.remove('active'));
      el.classList.add('active');
      try{ renderHomeRecent(cat); }catch(_){}
    });
  });
}
