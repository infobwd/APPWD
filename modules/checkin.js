...
function ensureLoginForActions(){
  ...
  const need = !profile || !profile.line_user_id;
  const btnGps = document.getElementById('btnGpsOnly');
  const btnScan = document.getElementById('btnOpenScanner');
  if(need){
    if(btnGps){ btnGps.setAttribute('disabled','disabled'); btnGps.classList.add('opacity-60','cursor-not-allowed'); }
    if(btnScan){ btnScan.setAttribute('disabled','disabled'); btnScan.classList.add('opacity-60','cursor-not-allowed'); }
    // Inject banner once
    ...
    if(parentCard && !document.getElementById('loginWarn')){
      ...
      warn.innerHTML = 'กรุณาเข้าสู่ระบบก่อนจึงจะเช็คอินได้';
      parentCard.insertBefore(warn, parentCard.children[1]); // after title line
    }
  }else{
    if(btnGps){ btnGps.removeAttribute('disabled'); btnGps.classList.remove('opacity-60','cursor-not-allowed'); }
    if(btnScan){ btnScan.removeAttribute('disabled'); btnScan.classList.remove('opacity-60','cursor-not-allowed'); }
...
async function loadToday(){  const profile = JSON.parse(localStorage.getItem('profile')||'null');
 const profile=JSON.parse(localStorage.getItem('LINE_PROFILE')||'null'); const box=document.getElementById('todayList'); if(!box)return; box.innerHTML=''; if(!profile){ box.innerHTML='<div class="text-ink3">ยังไม่เข้าสู่ระบบ</div>'; return; } const start=new Date(); start.setHours(0,0,0,0); const baseQuery = supabase.from('checkins').select('*').gte('created_at', start.toISOString()).lt('created_at', new Date(end.getTime()+1).toISOString());
  const query = (checkinScope==='mine' && profile?.line_user_id) ? baseQuery.eq('line_user_id', profile.line_user_id) : baseQuery;
