...
// === Apps/Systems card on Home — reads from app_links ===
  ...
  apps.forEach(a => {
    ...
    const el = mk(`<a class="block p-3 rounded-xl border border-slate-200 hover:border-blue-400 hover:shadow transition bg-white"
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="min-w-0">
            <div class="font-medium truncate">${safe(a.title||'ไม่ระบุชื่อ')}</div>
            ${a.category ? `<div class="text-xs text-slate-500 truncate">${safe(a.category)}</div>` : ''}
          </div>
        </div>
      </a>`);
    grid.appendChild(el);
