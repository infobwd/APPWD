...
export async function renderAppsCard(targetId='homeLinks'){
  ...
  try{
    const {data,error}=await supabase.from('app_links').select('id,title,url,image_url,category,sort_order,is_active').eq('is_active',true).order('sort_order',{ascending:true}).order('id',{ascending:true});
    ...
    if(isMobile()){
      ...
      el.innerHTML = `<div class="tiles-slider">${slides}</div><div class="dots">${dots}</div>
      ...
      const grid=rows.map(itemRow).join('');
      el.innerHTML = `<div class="links-grid">${grid}</div>
        <div class="links-more"><a class="btn btn-prim" href="https://infobwd.github.io/APPWD/#links">ดูแอปทั้งหมด</a></div>`;
    }
