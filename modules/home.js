...
function mk(h){ const t=document.createElement('template'); t.innerHTML=h.trim(); return t.content.firstElementChild; }
...
export async function renderAppsCard(containerId='homeLinks'){
  ...
  const apps=await loadFeaturedApps(12);
  if(!apps.length){ host.innerHTML=`<div class="p-4 text-slate-400">ยังไม่มีรายการ</div>`; return; }
  host.innerHTML = `<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3" id="appsGrid"></div>`;
