...
function pickDesc(row){ const cand=[row?.summary,row?.desc,row?.description,row?.content,row?.body,row?.content_md];
  const s=(cand.find(v=>typeof v==='string'&&v.trim())||'').replace(/<[^>]+>/g,'').replace(/\s+/g,' ').trim(); return s.slice(0,140); }
function pickCover(row){ return row?.cover_url || row?.image_url || row?.cover || row?.thumb || ''; }

async function fetchPost(id){ try{
