...
export async function renderHome(){ const listEl=document.getElementById('homeNewsList'); const cardsEl=document.getElementById('homeNewsCards'); if(!listEl||!cardsEl)return;
  ...
  listEl.innerHTML=top2.map(p=>`<div class='p-3 border rounded-xl bg-[var(--card)] flex items-center gap-3' style='border-color:var(--bd)'><a class='flex-shrink-0' href='#post?id=${p.id}'>${p.cover_url?`<img class='w-16 h-16 object-cover rounded-lg border' src='${p.cover_url}'>`:`<div class='w-16 h-16 rounded-lg bg-brandSoft grid place-items-center text-brand'>📰</div>`}</a><div class='flex-1'><a href='#post?id=${p.id}' class='font-semibold leading-snug line-clamp-2' style='color:var(--ink)'>${esc(p.title)}</a><div class='text-[12px] text-ink3'>${esc(p.category||'ทั่วไป')} • ${new Date(p.published_at).toLocaleDateString('th-TH')}</div></div></div>`).join('')||'<div class="text-ink3">ยังไม่มีข่าว</div>';
  const ids=picked.map(p=>p.id); const statMap=await fetchStats(ids);
  cardsEl.innerHTML=picked.map(p=>{ const s=statMap.get(p.id)||{views:0,likes:0}; return `<div class='news-card'><a href='#post?id=${p.id}'>${p.cover_url?`<img class='thumb' src='${p.cover_url}'>`:`<div class='thumb grid place-items-center bg-brandSoft text-brand'>📰</div>`}</a><div class='badge'>เด่น</div><div class='p-3'><div class='text-[12px] text-ink3 mb-1'>${esc(p.category||'ทั่วไป')} • ${new Date(p.published_at).toLocaleDateString('th-TH')}</div><a class='font-semibold leading-snug line-clamp-2 block' href='#post?id=${p.id}' style='color:var(--ink)'>${esc(p.title)}</a><div class='flex items-center gap-3 mt-2 text-[12px] text-ink2'><span>👁️ ${s.views}</span><span>❤️ ${s.likes}</span><button onclick='sharePost(${p.id})' class='underline'>แชร์</button></div></div></div>`; }).join('');
...
async function loadPage(p){ page=p; const box=document.getElementById('newsList'); if(!box)return; box.innerHTML=skel(PAGE_SIZE,'72px');
  ...
  box.innerHTML=data.map(p=>{ const s=statMap.get(p.id)||{views:0,likes:0}; const tools=can?`<div class='flex gap-2 mt-1'><button class='btn text-xs' onclick='editPost(${p.id})'>แก้ไข</button><button class='btn text-xs' onclick='deletePost(${p.id})'>ลบ</button></div>`:''; return `<article class='p-3 border rounded-xl bg-[var(--card)] flex items-center gap-3' style='border-color:var(--bd)'><a class='flex-shrink-0' href='#post?id=${p.id}'>${p.cover_url?`<img class='w-16 h-16 object-cover rounded-lg border' src='${p.cover_url}'>`:`<div class='w-16 h-16 rounded-lg bg-brandSoft grid place-items-center text-brand'>📰</div>`}</a><div class='flex-1'><a href='#post?id=${p.id}' class='font-semibold leading-snug line-clamp-2' style='color:var(--ink)'>${esc(p.title)}</a><div class='text-[12px] text-ink3'>${esc(p.category||'ทั่วไป')} • ${new Date(p.published_at).toLocaleDateString('th-TH')}</div><div class='flex items-center gap-3 mt-1 text-[12px] text-ink2'><span>👁️ ${s.views}</span><span>❤️ ${s.likes}</span><button onclick='sharePost(${p.id})' class='underline'>แชร์</button></div>${tools}</div></article>`; }).join('')||'<div class="text-ink3">ยังไม่มีข่าว</div>';
...
export async function renderDetail(id){ const box=document.getElementById('postDetail'); if(!box)return; box.innerHTML=skel(4,'80px'); const resp=await supabase.from('posts').select('id,title,category,body,cover_url,published_at,created_by,is_featured').eq('id',id).maybeSingle(); const p=resp.data; if(!p){ box.innerHTML='<div class="text-ink3">ไม่พบข่าว</div>'; return; }
  ...
  box.innerHTML=`${cover}<h1 class='text-xl font-semibold mb-1'>${esc(p.title)}</h1><div class='text-xs text-ink3 mb-3'>${esc(p.category||'ทั่วไป')} • ${d}</div><div class='prose prose-sm max-w-none mb-4' style='color:var(--ink)'>${safe}</div><div class='flex items-center gap-2'><button id='btnLike' class='btn' aria-pressed='${liked}'>${liked?'❤️':'🤍'} <span id='likeCount' class='ml-1'>${likes}</span></button><button id='btnShare' class='btn'>แชร์ LINE</button><div class='ml-auto text-sm text-ink3'>เปิดอ่าน <span id='viewCount'>${views}</span> ครั้ง</div></div>${can?`<div class='mt-3 flex gap-2'><button class='btn btn-prim' id='editP'>แก้ไข</button><button class='btn' id='delP'>ลบ</button></div>`:''}`;
  const editBtn=document.getElementById('editP'), delBtn=document.getElementById('delP'); if(editBtn) editBtn.onclick=()=>openEditSheet(p); if(delBtn) delBtn.onclick=async()=>{ if(!confirm('ลบข่าวนี้?'))return; const del=await supabase.from('posts').delete().eq('id',p.id); if(del.error){toast('ลบไม่สำเร็จ');return;} location.hash='#news'; };
  const likeBtn=document.getElementById('btnLike'); if(likeBtn){ likeBtn.onclick=async()=>{ if(!lineId){toast('กรุณาเข้าสู่ระบบด้วย LINE ก่อน');return;} const pressed=(likeBtn.getAttribute('aria-pressed')==='true'); try{ if(pressed){ const res=await supabase.rpc('unlike_post',{p_post_id:p.id,p_line_user_id:lineId}); likeBtn.setAttribute('aria-pressed','false'); likeBtn.firstChild.nodeValue='🤍'; document.getElementById('likeCount').textContent=(res.data||0);} else { const res=await supabase.rpc('like_post',{p_post_id:p.id,p_line_user_id:lineId}); likeBtn.setAttribute('aria-pressed','true'); likeBtn.firstChild.nodeValue='❤️'; document.getElementById('likeCount').textContent=(res.data||0);} }catch{ toast('ดำเนินการไม่สำเร็จ'); } }; }
...
window.sharePost=async function(id){ const base=(localStorage.getItem('APPWD_PUBLIC_URL')||'./'); const row=await supabase.from('posts').select('id,title,category,cover_url').eq('id',id).maybeSingle(); const p=row.data; if(!p){ toast('ไม่พบบทความ'); return; } const url=base+`index.html#post?id=${p.id}`; const bubble={type:'bubble',hero:p.cover_url?{type:'image',url:p.cover_url,size:'full',aspectRatio:'16:9',aspectMode:'cover'}:undefined,body:{type:'box',layout:'vertical',contents:[{type:'text',text:p.title||'ข่าว',weight:'bold',size:'md',wrap:true},{type:'text',text:(p.category||'ทั่วไป'),size:'xs',color:'#6B7280',wrap:true,margin:'sm'}]},footer:{type:'box',layout:'vertical',spacing:'sm',contents:[{type:'button',style:'primary',height:'sm',action:{type:'uri',label:'อ่านข่าว',uri:url}}]}}; try{ if(window.liff&&window.liff.isApiAvailable&&window.liff.isApiAvailable('shareTargetPicker')){ await window.liff.shareTargetPicker([{type:'flex',altText:`แชร์ข่าว: ${p.title}`,contents:bubble}]); } else { await navigator.clipboard.writeText(url); toast('คัดลอกลิงก์แล้ว'); } }catch(e){ toast('แชร์ไม่สำเร็จ'); } };
...
}; const ins=await supabase.from('posts').insert(payload).select('id').single(); if(ins.error){ toast('บันทึกข่าวไม่สำเร็จ'); return; } closeSheet(); location.hash=`#post?id=${ins.data.id}`; }; } }
function openEditSheet(p){
  ...
  const del=document.getElementById('delPost'); if(del) del.onclick=async()=>{ if(!confirm('ลบข่าวนี้?'))return; await supabase.from('posts').delete().eq('id',p.id); closeSheet(); location.hash='#news'; };
