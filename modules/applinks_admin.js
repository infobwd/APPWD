...
export async function renderAppLinksAdmin(containerId='applinksAdmin'){
  ...
  const listEl=document.getElementById('applinksList'), editorEl=document.getElementById('applinksEditor');
  async function refresh(){ const rows=await fetchLinks(); listEl.innerHTML=rows.map(listItemHTML).join('')||'<div class="text-slate-500">ยังไม่มีรายการ</div>'; listEl.querySelectorAll('[data-edit]').forEach(btn=>{ btn.addEventListener('click', async ()=>{ const id=Number(btn.getAttribute('data-edit')); const row=(await fetchLinks()).find(r=>r.id===id); editorEl.innerHTML=formHTML(row); wireForm(editorEl.querySelector('form')); }); }); }
  function wireForm(form){ form.addEventListener('submit', async e=>{ e.preventDefault(); const fd=new FormData(form); const payload={ id: form.getAttribute('data-id')?Number(form.getAttribute('data-id')):undefined, title: fd.get('title'), category: fd.get('category')||null, url: fd.get('url'), image_url: fd.get('image_url')||null, sort_order: Number(fd.get('sort_order')||100), is_active: fd.get('is_active')?true:false }; await upsertLink(payload); toast('บันทึกสำเร็จ','success'); editorEl.innerHTML=''; await refresh(); }); const delBtn=form.querySelector('[data-delete]'); if(delBtn){ delBtn.addEventListener('click', async ()=>{ const id=Number(delBtn.getAttribute('data-delete')); if(!confirm('ยืนยันการลบรายการนี้?')) return; await deleteLink(id); toast('ลบแล้ว','success'); editorEl.innerHTML=''; await refresh(); }); } }
