export function toast(m){const b=document.getElementById('toast');if(!b)return;b.classList.remove('hide');b.innerHTML=`<div class="rounded-xl px-3 py-2 shadow bg-white border" style="border-color:var(--bd);color:var(--ink)">${m}</div>`;clearTimeout(window.__t);window.__t=setTimeout(()=>b.classList.add('hide'),2600);}
